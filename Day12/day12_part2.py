...
start = find_all(height_map, "S")[0]
target = find_all(height_map, "E")[0]

height_map[start[0]][start[1]] = "a"
height_map[target[0]][target[1]] = "z"

possible_starts = find_all(height_map, "a")

print(min(filter(lambda v: v > 0, [path_to(height_map, start, target) for start in possible_starts])))
